...
@router.patch("/api/member", response_class=JSONResponse, tags=["updatename"])
async def update_name(request: Request):
    ...
    if "name" not in jsonData:
        return JSONResponse({"error":True})
    jsonRes=UserData.update_user(jsonData["name"],request.session["User"][0])
    u=UserData.query_username(request.session["User"][2])
    request.session["User"]=[u.id,u.name,u.username]
